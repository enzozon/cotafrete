...
def servidor(tmp_path_factory):
    ...
    banco.salvar_resultado(cid, "camilo", status="cotado", valor="69.91",
    ...
    banco.salvar_resultado(cid, "translovato", status="cotado", valor="112.50",
                           prazo="4 dias úteis")
...
def _telas(base: str, cotacao_id: int) -> tuple[tuple[str, str], ...]:
    ...
    return (
        ...
        ("aceitar-generoso", f"{base}/aceitar/{cotacao_id}/generoso"),
...
def _abrir(navegador, url: str, largura: int, altura: int, *,
    ...
    from web import app as modulo
    biscoitos = [{"name": modulo.COOKIE,
                  "value": sessao.assinar("enzo", modulo.banco.segredo_sessao()),
                  "domain": dominio, "path": "/"}]
    if adm:
...
def test_grava_os_prints_da_documentacao(servidor, navegador, perfil, largura,
    ...
    for nome, url in _telas(base, cid):
        ...
        pagina = ctx.new_page()
        pagina.goto(url, wait_until="networkidle")
        assert pagina.url == url, f"Tela {nome} redirecionou para {pagina.url}"
