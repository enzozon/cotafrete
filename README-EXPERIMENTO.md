...
Branch: `rebranding/claude-contexto-enxuto` · base: `806f1ff` · 15/09/2026
...
ligado × ECC e Superpowers desligados, como manda a seção 3 do guia). O
objetivo não era a tela bonita: era medir o que cada configuração cobra pelo
mesmo trabalho.
...
## O que o guia manda, e o que foi aplicado

A seção 3 diz para desativar sem apagar:
...
| teto de orçamento | esta branch rodou com `--max-budget-usd 14.45`, o que o braço 3 gastou, para não levar vantagem por pegar uma janela de uso limpa |

O teto não foi atingido: a corrida parou em US$ 12,74, cortada pelo limite da
conta antes disso.
...
O guia entrega o que promete nesta medida. A seção seguinte mostra o que isso
vale — e o que não vale — numa tarefa longa.
...
Diferença de ~3%, dentro do ruído de uma corrida só. Esta branch gastou 12%
menos no total simplesmente porque fez 12% menos: 112 turnos contra 133.

O contexto fixo menor **não** virou economia proporcional na tarefa longa, e o
motivo está na tabela: numa corrida de 16 minutos o cache lido chega a 17
milhões de tokens, e ao lado disso os ~10 mil tokens de catálogo economizados
por sessão somem. A economia dos 41,8% é real — ela aparece no **custo de
abrir sessão**, não no custo de trabalhar.

É exatamente o que o guia avisa no fim da seção 6: menos contexto não implica a
mesma redução na cobrança. O experimento confirma o aviso, não a esperança.
...
- **Esta branch escreveu o teste — e ele não roda.** As 277 linhas de
  ...
  saem todos daí; nenhum vem do código de produção.
- **O braço com tudo ligado não escreveu teste nenhum** e ignorou o requisito.

Nenhum dos dois cumpriu. Um ignorou, este entregou quebrado — e um teste que
não roda é pior que nenhum, porque parece cobertura.

O erro único do outro braço não é regressão: é o `test_braspress_dom.py`, que
passa isolado e falha na suíte cheia por dependência de ordem, defeito anterior
ao rebranding.

### O que isso NÃO diz

- Não diz quanto custa o rebranding: nenhum braço terminou.
- Não diz que um resultado visual é melhor. A comparação visual exige as
  corridas completas.
...
contra os −41,8% medidos aqui. O arnês tem uma `tentativa-invalida-*`
registrada e uma rotina `--diagnose --skills-filter` no `run_experiment.py`,
sinal de que fazer a desativação valer ali deu trabalho. Confirme que pegou
antes de concluir qualquer coisa sobre o Codex.
...
`web/app.py` (+1023 −224) e `tests/test_rebranding_visual.py` (277 linhas, que
não roda). Tarefa interrompida, não entrega pronta: faltaram
`docs/rebranding/IMPLEMENTACAO.md` e as capturas que o enunciado pedia.
