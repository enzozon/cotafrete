...
**−8,3%**, contra os −41,8% medidos no Claude. O arnês do Codex tem uma
tentativa anterior registrada como `tentativa-invalida-*` e uma rotina
`--diagnose --skills-filter` no `run_experiment.py`, sinal de que fazer a
desativação valer de fato foi difícil ali. **O guia não rende igual nas duas
ferramentas.**
